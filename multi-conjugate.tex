...
$100\times 100 \times 100 \mu m $ volume at a $1\mu m$ pitch would require approximately 10 minutes, which is too slow for high-throughput applications.
...
Based on the preceding analysis, we plan to pursue two  approaches for constructing a multi-conjugate correction system. These orthogonal approaches will help mitigate the project's risk by ensuring a viable path to volumetric correction.
...
\boldstart{Preliminery design:} 
We have  designed a first prototype for our relayed-MPLC  unit using careful ray tracing simulations of all optical paths. The lateral intersection of the multiple ray bounces with the SLM plane is visualized in \figref{fig:setup_rMPLC}(b).
 The design aims to correct all wavefronts emerging from fluorescent sources spread inside a FOV of $50\mu m \times 50\mu m$ in the tissue. We assume the tissue thickness is $250\mu m$ and aim to create $5$ equally spaced correction planes in this range. We plan to use a $\NA=0.5$ objective, $10\times$ magnification between the sample and the SLM plane, followed by $2\times$ magnification between the SLM plane and the sensor. We plan to use a standard Holoeye SLM whose area is $15.42\times 9.66 mm$.
